...
export const usePaginate = ({ totalPages }: { totalPages: number }) => {
  ...
  const frameNo = upper(currentIdx / LEN)
  let [left, right] = [0, 0]
  left = max(0, frameNo - 1) * 5
  right = left + LEN - 1
  console.log({ position, frameNo })

  if (isFirst(position) && frameNo != 0) {
    console.log("A")
    left = max(0, currentIdx - 2)
    right = left + LEN - 1
  }
  if (isLast(position) && frameNo != totalPages) {
    console.log("B")
    left = currentIdx - 2
    right = left + LEN - 1
  }
